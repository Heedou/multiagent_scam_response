...
from os import walk
import pdb
from scamcatcher.dataset import RawDataset
from scamcatcher.pipeline import ScamcathcerPipeline, true_or_false, classification_scores, bool_type_transform
import logging
...
if __name__ == '__main__':
    ...
    logging.info(f"STEP [2] : Preparing the Scamcatcher Pipeline to be evaluated >>>>> {conf.finetune.llm_backbone}")
    pipeline        = ScamcatcherPipeline(conf, logging)
    ...
    logging.info(f"STEP [3] : Running Inference on the Scamcatcher Pipeline >>>>> {conf.finetune.llm_backbone}")
    START_FLAG  = False
