...
class ScamcatcherTrainer(object):
    def __init__(self, conf, logging):
        ...
        self.path_dataset_train = os.path.join(path_dataset,
                                               f"{conf.dataprep.raw_dataset_train}_{conf.dataprep.subsample}.hf")
        self.path_dataset_dev = os.path.join(path_dataset,
                                             f"{conf.dataprep.raw_dataset_dev}_{conf.dataprep.subsample}.hf")
    ...
    def __call__(self, model, tokenizer):
        ...
        """llama 3.1 task1 학습 시 oom 발생 방지"""
